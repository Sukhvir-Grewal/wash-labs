...
// Helper to get JWT_SECRET and validate it's set (only at runtime, not build time)
 ...
 * @param {string} token - JWT token
 * @returns {object} Session info or false if invalid
 */
export function verifySession(token) {
  ...
  try {
    ...
    // Add expiry and needs refresh info
    const now = Math.floor(Date.now() / 1000);
    const exp = decoded.exp || 0;
    const needsRefresh = exp - now < REFRESH_THRESHOLD;
